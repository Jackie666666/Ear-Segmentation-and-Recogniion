...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    not_ignore_mask =tf.to_float(tf.equal(scaled_labels, 0)) * loss_weight_0 + tf.to_float(tf.equal(scaled_labels, 1)) * loss_weight_1 + tf.to_float(tf.equal(scaled_labels, ignore_label    )) * 0
